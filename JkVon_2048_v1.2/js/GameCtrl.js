...
function processBlock(block0_X, block0_Y, block1_X, block1_Y, block2_X, block2_Y, block3_X, block3_Y) {
    ...
    for(i = 3; i > -1; i--){
        if(!gameBlock.isBlockHided(blockBox[i])){
            k = i;
            for(j = k - 1; j > -1; j--){
                if(!gameBlock.isBlockHided(blockBox[j])){
    ...
    if( i === 3 && j === 2 ){
        if(!gameBlock.isBlockHided(blockBox[0]) && !gameBlock.isBlockHided(blockBox[1])){
...
function __XXX__processBlock(block0_X, block0_Y, block1_X, block1_Y, block2_X, block2_Y, block3_X, block3_Y) {
    ...
    if(!gameBlock.isBlockHided(block3_X, block3_Y)){
        if(!gameBlock.isBlockHided(block2_X, block2_Y)){
            if(gameBox.blockEqual(block2_X, block2_Y, block3_X, block3_Y)){
                gameBox.combineBlock(block2_X, block2_Y, block3_X, block3_Y);
                result.combineHappen++;
                if(!gameBlock.isBlockHided(block1_X, block1_Y)){
                    if(!gameBlock.isBlockHided(block0_X, block0_Y)){
...
function processDirectionKey(event) {
    ...
    if ( (result.moveHappen > 0) || (result.combineHappen > 0) ){
        ...
        //超时调用，是新生成的块晚一点出现，如过延时太长会导致更不上操作
        setTimeout("gameBox.randomCreateBlock()",80);
...
/* 绑定、添加鼠标click事件 */
EventUtil.addHandler(document.getElementById("menuBox"), "click" ,function (event) {
    event = EventUtil.getEvent(event);
    var target = EventUtil.getTarget(event);
